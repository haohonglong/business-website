		...
		editor.addCommand('simage', {
			exec: function (editor) {
				...
				a.onchange = function(){
					...
					loaderHtmlStr = '<div style="position: relative; z-index: 100;width: 100%;height: 100%;text-align: center;background: white;opacity: 0.75;pointer-events:none">' + '<div style="width: 100%;height: 30px;margin-top: 100px;">图片上传中……</div>' + '</div>'
					...
					$.ajax({
						...
						return function(data, textStatus, jqXHR) {
							...
							if (jqXHR.status == 200) {
								...
								newLine = CKEDITOR.dom.element.createFromHtml('<br/>')
								if (editor.config.srcSet){
									...
									imgElem = '<img src="' + url + '" style="height:' + height + 'px; width:' + width + 'px" />'
								}		
